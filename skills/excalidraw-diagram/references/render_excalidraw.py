...
def render(
    ...
    # Serve the template over http:// (not file://) so its ES module import
    # of the vendored excalidraw.bundle.mjs (and the fonts/ directory) works —
    # Chromium blocks module imports from file:// origins, which hangs
    # window.__moduleReady. Everything is served from references/ locally;
    # no network egress is required.
    bundle_path = template_path.parent / "excalidraw.bundle.mjs"
    if not bundle_path.exists():
        print(f"ERROR: Offline Excalidraw bundle not found at {bundle_path}", file=sys.stderr)
        print("The skill ships with a vendored excalidraw.bundle.mjs. To rebuild it:", file=sys.stderr)
        print("  npm install @excalidraw/excalidraw@0.18.1 esbuild", file=sys.stderr)
        print("  echo 'export { exportToSvg } from \"@excalidraw/excalidraw\";' > entry.js", file=sys.stderr)
        print("  npx esbuild entry.js --bundle --format=esm --minify \\", file=sys.stderr)
        print("    --define:process.env.NODE_ENV='\"production\"' --outfile=excalidraw.bundle.mjs", file=sys.stderr)
        sys.exit(1)
    import http.server, socketserver, threading, functools
    _handler = functools.partial(http.server.SimpleHTTPRequestHandler, directory=str(template_path.parent))
    _httpd = socketserver.TCPServer(("127.0.0.1", 0), _handler)
    _port = _httpd.server_address[1]
    threading.Thread(target=_httpd.serve_forever, daemon=True).start()
    template_url = f"http://127.0.0.1:{_port}/{template_path.name}"
    ...
    with sync_playwright() as p:
        ...
        # Wait for the ES module to load (vendored local bundle)
